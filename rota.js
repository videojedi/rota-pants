        ...
        function goToNextIssueWeek() {
            // Save current week before changing
            saveToLocalStorage();

            // Search up to 52 weeks ahead for a week with issues
            ...
            for (let i = 1; i <= maxWeeks; i++) {
                ...
                const weekKey = getWeekKey(checkWeek);
                const saved = localStorage.getItem('staffRota_' + weekKey);

                if (!saved) {
                    // Empty week = has issues
                    currentWeekStart = checkWeek;
                    loadWeekData();
                    updateWeekDisplay();
                    renderTable();
                    return;
            ...
            // No issues found in next 52 weeks
            alert('No weeks with issues found in the next 52 weeks!');
        }

        function weekHasIssues(weekRota) {
            // Check if any staff member has blank (unfilled) shifts
            for (const staffId of Object.keys(weekRota)) {
                for (const day of DAYS) {
                    const shift = weekRota[staffId]?.[day];
            ...
            // Check for coverage gaps (times with no staff)
            for (const day of DAYS) {
                for (let t = OPEN_TIME; t < CLOSE_TIME; t += 0.5) {
                    ...
                    for (const staffId of Object.keys(weekRota)) {
                        ...
                        if (shift && shift !== 'off') {
                            ...
                            if (shiftDef && t >= shiftDef.start && t < shiftDef.end) {
                                count++;
                            }
                        }
                    }
            ...
            // Check if any staff member doesn't have target hours
            for (const staffId of Object.keys(weekRota)) {
                let hours = 0;
                for (const day of DAYS) {
                    const shift = weekRota[staffId]?.[day];
        ...
        function showGenerationResult(result) {
            ...
            el.style.background = ''; // Clear any inline styles
            ...
            if (result.errors.length > 0) {
                ...
                el.innerHTML = `
                    <details>
                        <summary>⚠️ Generation completed with ${result.errors.length} error(s) and ${result.warnings.length} warning(s) - click for details</summary>
                ...
                el.innerHTML = `
                    <details>
                        <summary>⚠️ Rota generated with ${result.warnings.length} warning(s) - click for details</summary>
                        <div class="details-list">
                ...
                el.innerHTML = `
                    <details>
                        <summary>✓ Rota generated successfully - click for details</summary>
                        <div class="details-list">
        ...
        // Final cleanup: ensure all staff have 37.5 hours and exactly 2 days off
        // For staff short on hours due to weekend constraints:
        // 1. Convert blank weekend slots to OFF
        // 2. Convert a weekday OFF to floating shift
        function markRemainingBlanksAsOff(state) {
            staff.forEach(s => {
                let currentHours = state.getHours(s.id);
                console.log(`${s.name}: ${currentHours}/${TARGET_HOURS} hours`);

                // Get current assignments
                const blanks = DAYS.filter(d => !state.getAssignment(s.id, d));
                const offDays = DAYS.filter(d => state.getAssignment(s.id, d) === 'off');
                const weekdayOffs = offDays.filter(d => ['mon', 'tue', 'wed', 'thu', 'fri'].includes(d));
                const weekendBlanks = blanks.filter(d => d === 'sat' || d === 'sun');

                // If staff is short on hours and has weekend blanks + weekday offs
                if (currentHours < TARGET_HOURS && weekendBlanks.length > 0 && weekdayOffs.length > 0) {
                    console.log(`${s.name}: Fixing hours - has ${weekendBlanks.length} weekend blanks, ${weekdayOffs.length} weekday offs`);

                    // First, mark weekend blanks as OFF
                    weekendBlanks.forEach(day => {
                        state.assignments[s.id][day] = 'off';
                        console.log(`Marked weekend blank as OFF for ${s.name} on ${day}`);
                    });

                    // Now convert weekday offs to floating shifts until we reach target hours
                    // Try to preserve consecutive days off by picking the most isolated off day first
                    const sortedWeekdayOffs = [...weekdayOffs].sort((a, b) => {
                        // Score based on adjacency to other off days (lower = more isolated = convert first)
                        const allOffs = [...offDays, ...weekendBlanks];
                        const aIdx = DAYS.indexOf(a);
                        const bIdx = DAYS.indexOf(b);

                        const aHasAdjacentOff = allOffs.some(d => {
                            const dIdx = DAYS.indexOf(d);
                            return d !== a && Math.abs(dIdx - aIdx) === 1;
                        });
                        const bHasAdjacentOff = allOffs.some(d => {
                            const dIdx = DAYS.indexOf(d);
                            return d !== b && Math.abs(dIdx - bIdx) === 1;
                        });

                        // Prefer to convert days that DON'T have adjacent offs (preserve consecutive)
                        if (aHasAdjacentOff && !bHasAdjacentOff) return 1;  // Keep a, convert b first
                        if (!aHasAdjacentOff && bHasAdjacentOff) return -1; // Convert a first
                        return 0;
                    });

                    for (const day of sortedWeekdayOffs) {
                        if (state.getHours(s.id) >= TARGET_HOURS) break;

                        // Convert this OFF day to floating
                        state.assignments[s.id][day] = 'floating';
                        state.staffHours[s.id] = (state.staffHours[s.id] || 0) + SHIFTS.floating.hours;
                        console.log(`Converted OFF to floating for ${s.name} on ${day}, now ${state.getHours(s.id)} hours`);
                    }
                }

                // Final pass: mark any remaining blanks as OFF for staff at target hours
                currentHours = state.getHours(s.id);
                if (currentHours >= TARGET_HOURS) {
                    ...
                    console.warn(`${s.name} still only has ${currentHours} hours after fixes`);
                }
            });
        }
        ...
        function generate10Weeks() {
            ...
            // Collect all warnings and errors with week info
            const allErrors = [];
            const allWarnings = [];
            results.forEach(r => {
                r.errors.forEach(e => allErrors.push(`Week ${r.week}: ${e}`));
                r.warnings.forEach(w => allWarnings.push(`Week ${r.week}: ${w}`));
            });
        ...
        function clearThisWeek() {
            ...
            el.style.display = 'block';
            el.style.background = '';
            el.className = 'generation-result generation-info';
            el.innerHTML = '✓ <strong>This week cleared.</strong> All shifts for the current week have been removed.';
        }

        function clearAllData() {
            if (confirm('Are you sure you want to clear ALL data? This will remove all weeks, staff names, and preferences. This cannot be undone.')) {
                // Clear all localStorage items for the app
                const keysToRemove = [];
                for (let i = 0; i < localStorage.length; i++) {
                    const key = localStorage.key(i);
                    if (key && key.startsWith('staffRota')) {
                ...
                el.style.background = '';
                el.className = 'generation-result generation-error';
                el.innerHTML = '✓ <strong>All data cleared.</strong> Staff names reset to defaults. All weeks and preferences removed.';
            }
        }
